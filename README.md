...
- **微步驟規則**：一步一動作（同一段表單、用一次「下一步」或「建立」收尾的算一步）、每步交代「做什麼→在哪做→看到什麼算成功」、每步都有卡關出口，出口只剩「找人問」時也要寫等回覆期間可以先做什麼。
...
└── skills/
    └── newbie-handbook/
        ...
        │   └── real-screenshots.md 畫面來源四層優先序、Playwright 實拍、圖上標記、四條邊界、登入後台兩階段、中英並列字樣、機器翻譯頁與逐字查核、截圖字高算法
        └── assets/
